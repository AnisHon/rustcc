...
impl Parser<'_> {
    fn check_string(&self) -> bool {
        match &self.ctx.stream.peek().kind {
    ...
    fn parse_primary_expr(&mut self) -> ParserResult<ExprKey> {
        let lo = self.ctx.stream.span();
        let kind = if let Some(ident) = self.consume_ident() {
            ...
            // 匹配失败，无法恢复，报错
            // println!("error: {:?}", self.ctx.stream.peek());
        ...
        let hi = self.ctx.stream.prev_span();
    ...
    fn parse_postfix_expr_suffix(&mut self, mut lhs: ExprKey) -> ParserResult<ExprKey> {
        use TokenKind::*;
        let lo = self.ctx.stream.span();
        loop {
    ...
    fn parse_postfix_expr(&mut self) -> ParserResult<ExprKey> {
    ...
    fn parse_unary_expr(&mut self) -> ParserResult<ExprKey> {
        let lo = self.ctx.stream.span();
    ...
    fn parse_cast_expr(&mut self) -> ParserResult<ExprKey> {
        let lo = self.ctx.stream.span();
    ...
    fn parse_multiplicative_expr(&mut self) -> ParserResult<ExprKey> {
        let lo = self.ctx.stream.span();
    ...
    fn parse_additive_expr_rhs(&mut self, lhs: ExprKey, lo: Span) -> ParserResult<ExprKey> {
        use TokenKind::*;
    ...
    /// additive-expression
    fn parse_additive_expr(&mut self) -> ParserResult<ExprKey> {
        let lo = self.ctx.stream.span();
    ...
    fn parse_shift_expr_rhs(&mut self, lhs: ExprKey, lo: Span) -> ParserResult<ExprKey> {
        use TokenKind::*;
    ...
    fn parse_shift_expr(&mut self) -> ParserResult<ExprKey> {
        let lo = self.ctx.stream.span();
    ...
    fn parse_relational_expr_rhs(&mut self, lhs: ExprKey, lo: Span) -> ParserResult<ExprKey> {
        use TokenKind::*;
    ...
    fn parse_relational_expr(&mut self) -> ParserResult<ExprKey> {
        let lo = self.ctx.stream.span();
    ...
    fn parse_equality_expr_rhs(&mut self, lhs: ExprKey, lo: Span) -> ParserResult<ExprKey> {
        use TokenKind::*;
    ...
    fn parse_equality_expr(&mut self) -> ParserResult<ExprKey> {
        let lo = self.ctx.stream.span();
    ...
    fn parse_and_expr_rhs(&mut self, lhs: ExprKey, lo: Span) -> ParserResult<ExprKey> {
        use TokenKind::*;
    ...
    fn parse_and_expr(&mut self) -> ParserResult<ExprKey> {
        let lo = self.ctx.stream.span();
    ...
    fn parse_exclusive_or_expr_rhs(&mut self, lhs: ExprKey, lo: Span) -> ParserResult<ExprKey> {
        use TokenKind::*;
    ...
    fn parse_exclusive_or_expr(&mut self) -> ParserResult<ExprKey> {
        let lo = self.ctx.stream.span();
    ...
    fn parse_inclusive_or_expr_rhs(&mut self, lhs: ExprKey, lo: Span) -> ParserResult<ExprKey> {
        use TokenKind::*;
    ...
    fn parse_inclusive_or_expr(&mut self) -> ParserResult<ExprKey> {
        let lo = self.ctx.stream.span();
    ...
    fn parse_logical_and_expr_rhs(&mut self, lhs: ExprKey, lo: Span) -> ParserResult<ExprKey> {
        use TokenKind::*;
    ...
    fn parse_logical_and_expr(&mut self) -> ParserResult<ExprKey> {
        let lo = self.ctx.stream.span();
    ...
    fn parse_logical_or_expr_rhs(&mut self, lhs: ExprKey, lo: Span) -> ParserResult<ExprKey> {
        use TokenKind::*;
    ...
    fn parse_logical_or_expr(&mut self) -> ParserResult<ExprKey> {
        let lo = self.ctx.stream.span();
        let rhs = self.parse_logical_and_expr()?;
    ...
    fn parse_conditional_expr(&mut self) -> ParserResult<ExprKey> {
        let lo = self.ctx.stream.span();
    ...
    pub(crate) fn parse_assign_expr(&mut self) -> ParserResult<ExprKey> {
        let lo = self.ctx.stream.span();
        let lhs_key = self.parse_conditional_expr()?;
        let assign_op = self.consume_assign_op();
        if assign_op.is_none() {
            return Ok(lhs_key); // 不是赋值表达式
        }

        let lhs = self.ctx.get_expr(lhs_key);
    ...
    fn parse_expr_rhs(&mut self, lhs: ExprKey, lo: Span) -> ParserResult<ExprKey> {
