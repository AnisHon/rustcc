...
use crate::parser::sema::scope::lookup::{
...
impl Sema<'_> {
    ...
    fn is_enum(&self, ty: TypeKey) -> bool {
        let ty = self.type_ctx.get_type(ty);
        ty.kind.is_enum()
    }

    /// decl 是否是 record ，如果不是返回 DeclNotMatch 错误
    fn is_record(&self, kind: RecordKind, ty: TypeKey) -> bool {
        let ty = self.type_ctx.get_type(ty);
        ty.kind
            .as_record()
    ...
    /// 在当前作用域插入 enum 声明
    pub fn insert_enum_decl(&mut self, name: Ident, span: Span) -> ParserResult<DeclKey> {
        // 查询同级是否已经存在声明
        let symbol = self.scope_mgr.lookup_local_tag(name.symbol);
        let ty = match symbol {
            Some(x) => {
                // 检查是否为同一个
            ...
            None => {
                // 不存在，构建类型
                let kind = self.type_ctx.new_enum();
        ...
        // 存入池子
        let decl_key = self.insert_decl(decl);

        // 插入符号表
        let enum_def = lookup_or_insert_decl(ctx, decl_key, ty, ScopeSource::Tag);

        // 设置 definition
        let decl = self.get_decl_mut(decl_key);
        match &mut decl.kind {
    ...
    /// 填充 record 的前向声明
    pub fn fill_record_fwd_ref(&mut self, definition: DeclKey, decls: Vec<DeclKey>) {
        for decl in decls.into_iter() {
            let decl = self.get_decl_mut(decl);
