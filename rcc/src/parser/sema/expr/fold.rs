...
use crate::constant::typ::CHAR_BITWIDTH;
use crate::parser::comp_ctx::CompCtx;
use crate::types::parser::ast::{
    exprs::{Constant, ExprKind, UnaryOpKind}, ExprKey,
    TypeKey,
};
...
fn make_size(ctx: &CompCtx, sz: usize, result_ty: TypeKey) -> Constant {
    ...
    Constant::Integer {
        value: APInt::new(*signed, bits, value),
    }
}
...
fn fold_unary(ctx: &CompCtx, op: UnaryOpKind, rhs_key: ExprKey) -> Option<Value> {
    ...
    match value {
        Constant::Float { value } => {
            match op {
                ...
                Not => Constant::Integer {
                    value: APInt::from_bool(!value.as_bool()),
                },
        ...
        // 能支持一个*运算符
        Constant::String { value } => match op {
            Deref => Constant::Integer {
                value: APInt::new(true, CHAR_BITWIDTH, value[u8]),
            },
            _ => return None,
        },
    }
