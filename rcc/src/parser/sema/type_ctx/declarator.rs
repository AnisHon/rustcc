...
impl Sema<'_> {
    ...
    fn resolve_decl_spec(&mut self, decl_spec: &DeclSpec) -> ParserResult<TypeKey> {
    ...
    pub fn resolve_declarator(&mut self, declarator: Declarator) -> ParserResult<DeclInfo> {
        let decl_spec = declarator.decl_spec;

        let mut ty = self.resolve_decl_spec(&decl_spec)?;

        // 反向解析
        for chunk in declarator.chunks.into_iter().rev() {
            let builder = match chunk.kind {
                DeclaratorChunkKind::Array { expr } => self.resolve_array(ty, expr)?,
                DeclaratorChunkKind::Pointer { type_quals } => Self::resolve_pointer(ty, type_quals),
                DeclaratorChunkKind::Function { param } => self.resolve_function(ty, param)?,
            };
            ty = self.ctx
                .type_ctx
                .build_type(builder)
    ...
    fn resolve_array(
        ...
        let size = match expr {
            ...
            Some(x) => self.resolve_array_size(x)?,
        };
    ...
    /// 解析数组大小
    /// todo 重构
    fn resolve_array_size(&mut self, expr: ExprKey) -> ParserResult<ArraySize> {
        let expr = self.ctx.pop_expr(expr);
        let expr_ty = self.ctx.type_ctx.get_type(expr.ty);
