...
use crate::parser::types::ast::decl::{Decl, DeclKind, StructOrUnion};
use crate::parser::types::ast::func::FuncDecl;
use crate::parser::types::common::Ident;
use crate::parser::types::decl_spec::{EnumSpec, Enumerator, ParamDecl, StorageSpec, StorageSpecKind, StructDeclarator, StructSpec};
use crate::parser::types::declarator::{Declarator, DeclaratorChunkKind, InitDeclarator};
use crate::parser::types::sema::decl::decl_context::DeclContextKind;
use crate::parser::types::sema::sema_type::{IntegerSize, Type, TypeKind};
...
impl Sema {
    pub fn act_on_init_declarator(&mut self, declarator: InitDeclarator) -> ParserResult<Rc<Decl>> {
        let kind = DeclKind::VarInit { eq: declarator.eq, init: declarator.init };
        let PartialDecl {
            storage,
            name,
            ty
    ...
    /// 解析枚举成员，插入符号表
    pub fn act_on_enumerator(&mut self, enumerator: Enumerator) -> ParserResult<Rc<Decl>> {
    ...
    /// 类型参数
    pub fn act_on_param_var(&mut self, declarator: Declarator) -> ParserResult<Rc<Decl>> {
        ...
        let decl = Rc::new(Decl { storage, name, kind, ty, span });
        Ok(decl)
    }
    
    /// 函数声明，添加函数声明和参数列表进入符号表
    pub fn act_on_func_decl(&mut self, func_decl: FuncDecl) -> ParserResult<Rc<Decl>> {
        ...
        match param {
            ...
            ParamDecl::Idents(x) => { // K&R函数声明
                ...
                for x in &x.idents {
                    ...
                    params.push(Rc::clone(decl));
                }

                for (_, decl) in name_map {
                    // 存在不存在的声明，出错
                    todo!()
                }
        ...
        let ret_ty = match &ty.kind {
            TypeKind::Function { ret_ty, .. } => Rc::clone(ret_ty),
            _ => unreachable!()
        };
        ...
        decl_context.insert(Rc::clone(&decl))?;
    ...
    pub fn act_on_finish_enum(&mut self, spec: EnumSpec) -> ParserResult<Rc<Decl>> {
        let decl_context = self.exit_decl();
