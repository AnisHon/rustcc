...
declaration
    : declaration_specifiers init_declarator_list_opt ';' {$$ = Declaration::make_declaration($1, $2);}
    ;
...
struct_declaration
    : specifier_qualifier_list struct_declarator_list ';' {$$ = StructDeclaration::make_struct_declaration($1, $2);}
    ;
...
compound_statement
    : '{' '}'                 {$$ = CompoundStatement::make_empty($1);}
    | '{' block_item_list '}' {$$ = CompoundStatement::make_expr($2);}
    ;
...
expression_statement
    : ';'             {$$ = ExpressionStatement::make_empty($1);}
    | expression ';'  {$$ = ExpressionStatement::make_expr($1);}
    ;

selection_statement
    : KEYWORD_IF '(' expression ')' statement                         {$$ = SelectionStatement::make_if($3, $5, SemanticValue::None);} %prec nonassoc
    | KEYWORD_IF '(' expression ')' statement KEYWORD_ELSE statement  {$$ = SelectionStatement::make_if($3, $5, $7);}
    | KEYWORD_SWITCH '(' expression ')' statement                     {$$ = SelectionStatement::make_switch($3, $5);}
    ;
...
jump_statement
    : KEYWORD_GOTO ID ';'             {$$ = JumpStatement::make_goto($2);}
    | KEYWORD_CONTINUE ';'            {$$ = JumpStatement::make_continue($1);}
    | KEYWORD_BREAK ';'               {$$ = JumpStatement::make_break($1);}
    | KEYWORD_RETURN ';'              {$$ = JumpStatement::make_return(SemanticValue::None);}
    | KEYWORD_RETURN expression ';'   {$$ = JumpStatement::make_return($2);}
    ;
